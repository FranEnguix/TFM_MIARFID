...
class FLAgent(Agent):
    def __init__(self, jid: str, password: str, port, dataset, model_type, neighbours, model_path):
        ...
        # Set coalition information in agent
        coalition_index = -1
        for i, coalition in enumerate(Config.coalitions):
            if self.name in coalition:
                self.coalition = [a for a in coalition if a != self.name]
                coalition_index = i
                break
        if coalition_index == -1:
            self.coalition = None
            print(f"{self.name} has not a coalition.")
        else:
            if self.dataset.startswith("cifar4_coal"):
                self.dataset = f"{self.dataset}|{self.name}"
            if Config.coalition_probability >= 0:
                print(f"{self.name} belongs to coalition number {coalition_index}, with agents: {self.coalition}")
    ...
    def agent_web_controller(self, request):
        ...
        return {"last_message": self.last_message, "train_acc": self.train_acc, "train_loss": self.train_loss,
                ...
                "received_message_statistics": active_neighbours_recv, "sent_message_statistics": active_neighbours_send,
                "available_agents": str_available_agents, "nb_available_agents": len(str_available_agents)}
    ...
    async def setup(self):
        ...
        self.state_machine_behaviour.add_transition(source=Config.RECEIVE_STATE_AG, dest=Config.RECEIVE_STATE_AG)
